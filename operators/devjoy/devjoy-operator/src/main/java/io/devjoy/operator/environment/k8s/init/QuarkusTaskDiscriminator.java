...
public class QuarkusTaskDiscriminator implements ResourceDiscriminator<Task, DevEnvironment>{
    ...
    public Optional<Task> distinguish(Class<Task> resource, DevEnvironment primary, Context<DevEnvironment> context) {
        return Optional.ofNullable(tektonClient.v1()
            .tasks().inNamespace(primary.getMetadata().getNamespace()).withName("quarkus-create").get());
    }
}
