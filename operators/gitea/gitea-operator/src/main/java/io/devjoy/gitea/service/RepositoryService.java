...
public class RepositoryService {
	...
	public void delete(GiteaRepository repository, String token) {
		try {
			...
			delete(repository.getSpec().getUser(), repository.getMetadata().getName(), token, baseUri);
		} catch (URISyntaxException | MalformedURLException e) {
			LOG.error("Error calling repository API to delete user", e);
		}
	}
	
	public void delete(String user, String repoName, String token, String baseUri) {
		try {
			getDynamicUrlClient(new URIBuilder(baseUri).build(), RepositoryApi.class, token).repoDelete(user, repoName);
		} catch (URISyntaxException e) {
			LOG.error("Error calling repository API to delete user", e);
		}
	}
