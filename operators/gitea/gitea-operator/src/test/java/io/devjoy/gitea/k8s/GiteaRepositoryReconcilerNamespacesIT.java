...
@QuarkusTest
class GiteaRepositoryReconcilerNamespacesIT {
	...
	static void tearDown() {
		client.resources(GiteaRepository.class).inNamespace(client.getNamespace() + "2").delete();
        client.resources(Gitea.class).inNamespace(client.getNamespace() + "2").delete();
	}
    ...
    GiteaRepository createDefaultRepo(String name) {
        ...
        repo.setMetadata(new ObjectMetaBuilder()
                .withName(name)
                .withNamespace(client.getNamespace() + "2")
                .build()); 
