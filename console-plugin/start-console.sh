...
CONSOLE_IMAGE=${CONSOLE_IMAGE:="quay.io/openshift/origin-console:4.13"}
...
if [ -x "$(command -v podman)" ]; then
    if [ "$(uname -s)" = "Linux" ]; then
        ...
        podman run --arch=amd64 --rm --network=host --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
    else
	BRIDGE_PLUGINS="${npm_package_consolePlugin_name}=http://host.containers.internal:9001"
	podman run --arch=amd64 --rm -p "$CONSOLE_PORT":9000 --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
    fi
else
    BRIDGE_PLUGINS="${npm_package_consolePlugin_name}=http://host.docker.internal:9001"
    docker run --rm --arch=amd64 -p "$CONSOLE_PORT":9000 --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
fi
